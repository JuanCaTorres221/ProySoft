...
public class EmotionController {
    ...
    @PostMapping
    public ResponseEntity<?> createEmotion(@Valid @RequestBody Emotion emotion, BindingResult result) {
        if (result.hasErrors()) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> updateEmotion(@PathVariable Long id, @Valid @RequestBody Emotion emotionData, BindingResult result) {
        if (result.hasErrors()) {
            return ResponseEntity.badRequest().body("Error de validacion");
        }
        try {
